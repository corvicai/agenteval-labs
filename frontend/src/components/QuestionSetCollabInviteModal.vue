    ...
    <div class="modal-container collab-invite-modal">
      ...
      <div class="modal-body">
        <p class="modal-subtitle">
          Invite someone to collaborate on <strong>{{ questionSetName }}</strong>.
          Collaborators can run benchmarks and create evaluations on this question set.
        </p>
        ...
        <div class="invite-form">
          ...
          <div class="form-group">
            ...
            <select v-model="inviteRole" class="form-select" :disabled="creating">
              <option value="editor">Editor (can run benchmarks and evaluate)</option>
          ...
          <div v-if="inviteLink" class="invite-link-box">
            ...
            <p class="invite-link-note">
              Link expires on {{ formatDate(inviteExpires) }}. Share it with your collaborator.
            </p>
          </div>
...
async function createInvite() {
  ...
  try {
    ...
    inviteLink.value = `${baseUrl}?collab_invite=${token}`
    inviteExpires.value = result.expires_at

    // Refresh collaborators list if it was loaded.
...
async function loadCollaborators() {
  ...
  try {
    const result = await wsService.listCollaborators(props.questionSetId)
    collaborators.value = result.collaborators || []
  } catch (err) {
    console.error('[CollabModal] Failed to load collaborators:', err)
    collaborators.value = []
  } finally {
...
async function revokeCollaborator(userId) {
  ...
  revokeError.value = ''
  try {
    await wsService.revokeCollaborator(props.questionSetId, userId)
    await loadCollaborators()
  } catch (err) {
...
onMounted(() => {
  // Pre-load collaborators to show current state.
  loadCollaborators()
})
</script>
