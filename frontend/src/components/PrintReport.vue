  ...
  <div class="print-report" :class="{ 'print-report-compact': isQuestionCardReport }">
    ...
    <template v-else>
      ...
      <div class="print-page cover-page">
        ...
        <div class="summary-section-print" v-if="summaryStats && (summaryStats.agents || summaryStats.notes)">
          <h2 class="section-title">Performance Executive Summary</h2>
          
          <div class="summary-grid-print" v-if="summaryStats.agents">
            <div 
          ...
          <div v-if="summaryStats.notes" class="report-notes-card">
            <h3>Benchmark Notes</h3>
            <div class="report-notes-content markdown-content" v-html="renderMarkdown(summaryStats.notes)"></div>
          </div>
...
.report-notes-card {
  margin-bottom: 2rem;
  padding: 22px 24px;
  background: linear-gradient(135deg, rgba(73, 57, 157, 0.06), rgba(118, 75, 162, 0.1));
  border: 1px solid rgba(73, 57, 157, 0.14);
  border-radius: 20px;
  box-shadow: 0 18px 45px rgba(73, 57, 157, 0.08);
}

.report-notes-card h3 {
  margin: 0 0 0.75rem;
  font-size: 1.05rem;
  font-weight: 700;
  color: var(--primary);
